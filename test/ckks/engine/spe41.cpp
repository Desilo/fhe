...
SETUP_ENGINE_TEST_SPE41(
    AddSubtractInplaceUnitPlaintextSPE41,
...
ALL_ENGINE_TEST(AddSubtractInplaceUnitPlaintextSPE41, AddTrio, {
  ...
  EXPECT_EQ(input_unit_ciphertext->level(), level);
  EXPECT_EQ(input_unit_ciphertext->flag(), expected_flag);
  EXPECT_EQ(input_unit_ciphertext->polynomial_count(), 3);
...
ALL_ENGINE_TEST(AddSubtractMessageSPE41, AddTrio, {
  const auto ciphertext_first = std::get<0>(GetParam());
  ...
  auto added = ciphertext_first ? engine.add(input_ciphertext, message)
...
ALL_ENGINE_TEST(AddSubtractMessageSPE41, SubtractTrio, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto slice_count = std::get<1>(GetParam()) ? 2 : 1;
  const auto ciphertext_ntt = std::get<2>(GetParam());
  const auto ciphertext_rescaled = std::get<3>(GetParam());
  ...
  auto ciphertext = engine.encrypt(message1, public_key_, level + 1);
  auto squared = engine.square(ciphertext);
...
ALL_ENGINE_TEST(AddSubtractMessageSPE41, AddTrioBuffer, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto slice_count = std::get<1>(GetParam()) ? 2 : 1;
  const auto ciphertext_ntt = std::get<2>(GetParam());
  const auto ciphertext_rescaled = std::get<3>(GetParam());
  ...
  auto ciphertext =
      engine.encrypt(message, public_key_, message_size, level + 1);
  auto squared = engine.square(ciphertext);
...
ALL_ENGINE_TEST(AddSubtractMessageSPE41, SubtractTrioBuffer, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto slice_count = std::get<1>(GetParam()) ? 2 : 1;
  const auto ciphertext_ntt = std::get<2>(GetParam());
  const auto ciphertext_rescaled = std::get<3>(GetParam());
  ...
  auto ciphertext =
      engine.encrypt(message1, public_key_, message_size, level + 1);
  auto squared = engine.square(ciphertext);
...
ALL_ENGINE_TEST(AddSubtractMessageSPE41, AddTrioDoubleScalar, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto slice_count = std::get<1>(GetParam()) ? 2 : 1;
  const auto ciphertext_ntt = std::get<2>(GetParam());
  const auto ciphertext_rescaled = std::get<3>(GetParam());
  ...
  auto added = ciphertext_first ? engine.add(input_ciphertext, 1.0)
...
ALL_ENGINE_TEST(AddSubtractMessageSPE41, SubtractTrioDoubleScalar, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto slice_count = std::get<1>(GetParam()) ? 2 : 1;
  const auto ciphertext_ntt = std::get<2>(GetParam());
  const auto ciphertext_rescaled = std::get<3>(GetParam());
  ...
  auto subtracted = ciphertext_first ? engine.subtract(input_ciphertext, 1.0)
...
ALL_ENGINE_TEST(MultiplyMessageSPE41, MultiplyTrio, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto slice_count = std::get<1>(GetParam()) ? 2 : 1;
  const auto ciphertext_ntt = std::get<2>(GetParam());
  const auto ciphertext_rescaled = std::get<3>(GetParam());
...
ALL_ENGINE_TEST(MultiplyMessageSPE41, MultiplyTrioBuffer, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto slice_count = std::get<1>(GetParam()) ? 2 : 1;
  const auto ciphertext_ntt = std::get<2>(GetParam());
  const auto ciphertext_rescaled = std::get<3>(GetParam());
