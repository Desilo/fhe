...
SETUP_ENGINE_TEST_SPE63(
    AddSubtractPlaintextSPE63,
...
ALL_ENGINE_TEST(AddSubtractPlaintextSPE63, AddTrio, {
  ...
  auto ciphertext = engine.encrypt(message, public_key_, level1 + 1);
...
ALL_ENGINE_TEST(AddSubtractPlaintextSPE63, SubtractTrio, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto ciphertext_ntt = std::get<1>(GetParam());
  const auto ciphertext_rescaled = std::get<2>(GetParam());
  const auto plaintext_ntt = std::get<3>(GetParam());
  ...
  auto ciphertext = engine.encrypt(message1, public_key_, level1 + 1);
  auto squared = engine.square(ciphertext);
...
ALL_ENGINE_TEST(MultiplyPlaintextSPE63, MultiplyTrio, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto ciphertext_ntt = std::get<1>(GetParam());
  const auto ciphertext_rescaled = std::get<2>(GetParam());
  const auto plaintext_ntt = std::get<3>(GetParam());
