...
ALL_ENGINE_TEST(AddSubtractSP53, AddDuo, {
...
ALL_ENGINE_TEST(AddSubtractSP53, SubtractDuo, {
  const auto first_ntt = std::get<0>(GetParam());
  const auto first_rescaled = std::get<1>(GetParam());
  const auto second_ntt = std::get<2>(GetParam());
  const auto second_rescaled = std::get<3>(GetParam());
...
ALL_ENGINE_TEST(AddSubtractUnitPlaintextSP53, AddDuo, {
  ...
  auto unit_ciphertext = engine.encrypt_unit(message, public_key_, level1);
...
ALL_ENGINE_TEST(AddSubtractUnitPlaintextSP53, SubtractDuo, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto ciphertext_ntt = std::get<1>(GetParam());
  const auto ciphertext_rescaled = std::get<2>(GetParam());
  const auto plaintext_ntt = std::get<3>(GetParam());
  ...
  auto unit_ciphertext = engine.encrypt_unit(message1, public_key_, level1);
...
ALL_ENGINE_TEST(MultiplySP53, MultiplyUnitPlaintext, {
  const auto ciphertext_first = std::get<0>(GetParam());
  const auto ciphertext_ntt = std::get<1>(GetParam());
  const auto ciphertext_rescaled = std::get<2>(GetParam());
  const auto plaintext_ntt = std::get<3>(GetParam());
